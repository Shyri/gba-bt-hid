...
#include "hid_device_le_profile.h"
#include <string.h>
#include "esp_log.h"
...
esp_err_t esp_hidd_register_callbacks(esp_hidd_event_cb_t callbacks) {
    ...
    // Store callbacks for HID Device
    if (callbacks != NULL) {
    ...
    // Register GATTS event callbacks
    if ((hidd_status = hidd_register_cb()) != ESP_OK) {
        return hidd_status;
    }

    // Register Battery Level Application Profile
    esp_ble_gatts_app_register(BATTRAY_APP_ID);

    // Register HID Application Profile
...
void
esp_hidd_send_keyboard_value(uint16_t conn_id, key_mask_t special_key_mask, uint8_t *keyboard_cmd, uint8_t num_key) {
